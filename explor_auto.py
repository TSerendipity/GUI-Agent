...
def tsk_setting(state: State):
    ...
    # print(f"tsk_setting:state['context']:{state['context']}")
...
def page_understand(state: State):
    ...
    """
    print("explor_auto:page_understand")
...
def perform_action(state: State):
    ...
    with open(json_labeled_path, "r", encoding="utf-8") as f:
        page_json = f.read()
    print(f"len of page_json:{len(page_json)}")
    ...
    # print(f"messages:{state["context"][-4:]}")
